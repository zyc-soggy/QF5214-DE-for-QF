...
def xueqiu(Start,End):   #两个参数分别表示开始读取与结束读取的页码
    comments_list = []  
    #将响应头的参数补齐以避免无法读取的情况
    headers = {"Refer":"https://xueqiu.com/k?q=TSLA",
           "Host": "xueqiu.com",
    ...
    for i in range(int(Start),int(End)+1):
        url='https://xueqiu.com/query/v1/search/status.json?sortId=2&q=TSLA&count=10''&page='+ str(i)
       # print(url)    用于检查
...
def output_csv(datalist):
    ...
    import csv
    csv_file = open("comments_data_TSLA.csv", 'a+', newline='', encoding='utf-8-sig')  # 解决中文乱码问题。a+表示向csv文件追加
